...
static inline void eval_error(const char *msg, const char *expr, int pos) {
    coc_log(COC_ERROR, "Eval error at pos %d in \"%s\": %s", pos, expr, msg);
    exit(1);
}
...
static inline Eval_Token eval_next_token(const char *s, int *pos) {
    ...
    if (strchr("+-*/%><#=!&|?:", c) == NULL) {
        coc_log(COC_ERROR,
                "Eval error at pos %d in \"%s\": unexpected character %c",
                *pos, s, c);
        exit(1);
    }
...
static inline Eval_Node *eval_new_node(Eval_NodePool *p, const char *expr, int pos) {
    if (p->used >= EVAL_NODE_MAX) eval_error("expression too complex", expr, pos);
...
static inline Eval_Node *eval_parse_factor(Eval_Parser *p) {
    ...
    if (t.kind == eval_tok_var) {
        eval_parser_take(p);
        Eval_Node *n = eval_new_node(p->pool, p->expr, cur_pos);
    ...
    eval_error("expected variable (A-Z, a-z) or digit (0-9)", p->expr, cur_pos);
}
...
static inline Eval_Node *eval_parse_unary(Eval_Parser *p) {
    ...
    if (t.kind == '!' || t.kind == '-') {
        eval_parser_take(p);
        Eval_Node *child = eval_parse_factor(p);
        Eval_Node *n = eval_new_node(p->pool, p->expr, cur_pos);
...
static inline Eval_Node *eval_parse_mul(Eval_Parser *p) {
    ...
    while (true) {
        ...
        Eval_Node *n = eval_new_node(p->pool, p->expr, op.pos);
...
static inline Eval_Node *eval_parse_ternary(Eval_Parser *p) {
    ...
    if (eval_parser_peek(p).kind != ':') {
        eval_error("expected ':'", p->expr, eval_parser_peek(p).pos);
    }
    eval_parser_take(p);
    Eval_Node *f = eval_parse_or(p);
    Eval_Node *n = eval_new_node(p->pool, p->expr, q.pos);
...
static inline double eval_node(const Eval_Node *n, const double vars[52], const char *expr) {
    switch (n->kind) {
        case EVAL_NODE_NUM: return (double)n->as.num.num;
        case EVAL_NODE_VAR: return vars[n->as.var.var];
        case EVAL_NODE_UNARY: {
            double a = eval_node(n->as.unary.a, vars, expr);
            if (n->as.unary.op == '!') return truthy(a) ? 0.0 : 1.0;
            if (n->as.unary.op == '-') return -a;
            eval_error("unknown unary op", expr, n->pos);
        }
        case EVAL_NODE_BINARY: {
            Eval_TokenKind op = n->as.binary.op;
            if (op == '&') {
                double l = eval_node(n->as.binary.l, vars, expr);
                if (!truthy(l)) return 0.0;
                double r = eval_node(n->as.binary.r, vars, expr);
            ...
            case '*': return l * r;
            case '/':
                if (r == 0.0) eval_error("division by zero", expr, n->pos);
                return l / r;
            case '%': {
                if (!isfinite(l) || !isfinite(r))
                    eval_error("mod expects finite numbers", expr, n->pos);
                int64_t a = (int64_t)l;
                int64_t b = (int64_t)r;
                if (b == 0) eval_error("mod by zero", expr, n->pos);
                return (double)(a % b);
            }
            ...
            case '#': return !is_equal_float(l, r) ? 1.0 : 0.0;
            default:
                eval_error("unknown binary op", expr, n->pos);
            }
        }
        case EVAL_NODE_TERNARY: {
            double cond = eval_node(n->as.ternary.c, vars, expr);
            if (truthy(cond)) return eval_node(n->as.ternary.t, vars, expr);
            return eval_node(n->as.ternary.f, vars, expr); 
        }
    }
    eval_error("unknown node kind", expr, n->pos);
    return 0.0;
}

static inline double eval_run(const char *expr, const double vars[52]) {
    Eval_Token tokens[EVAL_TOKEN_MAX];
    int token_cnt = 0, pos = 0;
    while (true) {
        Eval_Token t = eval_next_token(expr, &pos);
        if (token_cnt >= EVAL_TOKEN_MAX) eval_error("too many tokens", expr, pos);
        tokens[token_cnt++] = t;
    ...
    if (eval_parser_peek(&p).kind != eval_tok_eof)
        eval_error("trailing garbage", expr, eval_parser_peek(&p).pos);
    return eval_node(root, vars, expr);
}
